...
def generate_location():
    ...
    if random.random() < 0.90:
        ...
        cidade = (
            random.choice(estado["cities"]) if estado and estado["cities"] else None
        )
...
def criar_transacao():
    ...
    card_number = [f"{random.randint(0, 9999999999999999):016d}" for _ in range(600)]
    ...
    estabelecimentos = [f"{random.randint(0, 99999999):08d}" for _ in range(300)]
    ...
    return {
        ...
        "bank": random.choice(["BANCO_A", "BANCO_B", "BANCO_C", "BANCO_D"]),
        "card_number": random.choice(card_number),
        "trn_dt": datetime.now(ZoneInfo("America/Sao_Paulo")).isoformat(),
        "transaction_type": "M"
        if random.random() < 0.92
        else random.choice(["O", "P", "A"]),
        "amount": amount,
        "card_limit_total": limit,
        "card_limit_remaining": card_limit_remaining,
        "process_code": "00"
        if random.random() < 0.90
        else f"{random.randint(1, 99):02d}",
        "num_reason": "000"
        if random.random() < 0.90
        else f"{random.randint(1, 999):03d}",
        "pos_number": "81"
        if random.random() < 0.55
        else random.choice(["01", "10", "05", "07", "90"]),
